...
setwd("C:\\...\\NO2_daily_2010_2020") #set working directory
...
all_stat_10_year_no2 = all_stat_10_year

save(all_stat_10_year_no2, file = "C:\\...\\all_stat_no2.RData")
...
ggplot(data = states_gg) +
  ...
  scale_size_continuous(range = c(1, 16),
                        label = scales::comma) +
  labs(title = "Available stations in the US for the NO2") +
...
ggplot(data = california) +
  ...
  scale_size_continuous(range = c(1, 16),
                        label = scales::comma) +
  labs(title = "Available stations in California for NO2") +
  theme(legend.position = "right") 
...
final_latlon_stats_no2 = final_latlon_stats

save(final_latlon_stats_no2, file = "C:\\...\\final_stat_no2.RData")
...
ggplot(data = states_gg) +
  ...
  scale_size_continuous(range = c(1, 16),
                        label = scales::comma)  +
  labs(title = "Selected NO2 Monitors in the US") +
  theme(legend.position = "right") 
...
final_latlon_stats_cali <- final_latlon_stats_no2  %>% filter((Lon < -116) & (Lon > -125) &  (Lat < 42) & (Lat > 32) )
...
gg = ggplot(data = california) +
  ...
  scale_size_continuous(range = c(5, 10),
                        label = scales::comma)  +
  labs(title = "Selected NO2 Monitors in California")  +
  theme_bw() +
...
ggsave("C:\\...\\cali_no2_stations.pdf",
       gg, width = 5 , height = 5)
...
sapply(1:n_states, function(i) unique(my_file_state[[i]]$`Sample Duration`))

sapply(1:n_states, function(i) unique(my_file_state[[i]]$`Pollutant Standard`))

# In this case I chose for Pollutant Standard there is only the option "NO2 1-hour "
# which gives the same results of Sample Duration: "1 HOUR" (only solution again) 
# Hence -->  my_file_state_1h == my_file_state
...
l_name_stations_no2 = l_name_stations
names_states_no2 = names_states

save(l_name_stations_no2, names_states_no2, file = "C:\\...\\hist_no2.RData" )

load("C:\\...\\hist_no2.RData")
...
ggsave("C:\\...\\no2_n_stations_US.pdf",
       gg, width = 18 , height = 16)
dev.off()


#########################################################
# Average the value of NO2 across stations per each day #   
#########################################################
...
no2_avg_st = lapply(1:n_states, function(i)
  my_file_state_1h[[i]] %>% group_by(`Date Local`) %>% dplyr::summarise(no2_avg = mean(`Arithmetic Mean`, na.rm=TRUE)))


final_no2avg_state = lapply(1:n_states, function(i) 
  data.frame(State = unique(my_file_state_1h[[i]]$`State Name`), no2_avg_st[[i]] ))


save(final_no2avg_state, my_file_state_1h, 
     file = "C:/.../no2_avg_files.RData")
...
cali_no2 = my_file_state_1h[[3]]

#Separate by counties

cali_no2_county<- cali_no2 %>% group_split(`County Name`)

names_counties =  sapply(1:length(cali_no2_county), function(i) unique(cali_no2_county[[i]]$`County Name`))

#note: need to aggregate them for the city or to select them for the city
sapply(1:length(cali_no2_county), function(i) unique(cali_no2_county[[i]]$`City Name`))
###############################################################################################
#same in AQI and CO and PM2.5

l_name_stations_cali<- lapply(1:length(cali_no2_county), function(i)  
  unique(cali_no2_county[[i]]$`Local Site Name` ))


n_stats_cali<- sapply(1:length(cali_no2_county), function(i) length(l_name_stations_cali[[i]]) )
...
no2_avg_st_cali = lapply(1:length(cali_no2_county), function(i)
  cali_no2_county[[i]] %>% group_by(`Date Local`) %>% dplyr::summarise(no2_avg = mean(`Arithmetic Mean`, na.rm=TRUE)))


final_no2avg_cali = lapply(1:length(cali_no2_county), function(i) 
  data.frame(County = unique(cali_no2_county[[i]]$`County Name`), no2_avg_st_cali[[i]] ))

save(final_no2avg_cali, cali_no2_county, 
     file = "C:/.../no2_avg_files_cali.RData")
